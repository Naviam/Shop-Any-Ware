    ...
    public class PackagesService : ServiceBase, IPackagesService
    {
        ...
        public ChangePackageDeliveryAddressResponse ChangePackageDeliveryAddress(ChangePackageDeliveryAddressRequest request)
        {
            try
            {
                ...
                return new ChangePackageDeliveryAddressResponse {CountryCode=addr.Country.Code, MessageType = MessageType.Success, Message = string.Format(CommonResources.PackageDeliveryAddressChanged, package.Id) };
            }
            catch (Exception ex)
            {
